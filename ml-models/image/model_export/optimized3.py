...
- Smart center fallback for messy backgrounds
...
class ChildDrawingPreprocessor:
    ...
    def process(self,
        ...
        paper = self._add_realistic_drawing_texture(paper)
    ...
    def _fast_edge_detection(self, img: np.ndarray) -> Optional[np.ndarray]:
        ...
        for contour in contours:
            ...
            # (not 10-90% like before - reduces false positives)
            if 0.3 < area_ratio < 0.85:
    ...
    def _validate_paper_mask(self, mask: np.ndarray, img_shape: Tuple[int, int]) -> bool:
        ...
        Enhanced validation with center-bias check
        """
        ...
        if rectangularity < 0.6:  # Should be reasonably rectangular
            return False
        ...
        if solidity < 0.75:  # Should be fairly convex
            return False

        # 5. CENTER-BIAS CHECK: Paper should be reasonably centered
        # (People naturally center drawings when photographing them)
        moments = cv2.moments(contour)
        if moments['m00'] > 0:
            # Calculate centroid of detected mask
            ...
            max_acceptable_dist = min(img_shape) * 0.45  # Within 45% of image dimension

            if dist_from_center > max_acceptable_dist:
                # Mask is too far off-center - probably detected wrong object
                return False
    ...
    #
    #     return result.astype(np.uint8)
    def _add_realistic_drawing_texture(self, img: np.ndarray) -> np.ndarray:
        """
        Add realistic paper grain and crayon texture to match training dataset

        Training images show:
        - Paper grain visible across entire image (including white areas)
        - Crayon stroke texture in colored areas
        ...
        # 1. PAPER GRAIN - applies to EVERYTHING (even white background)
        # This is the texture you see across the whole scanned paper
        paper_grain = np.random.normal(0, 1.5, (h, w)).astype(np.float32)
        paper_grain = cv2.GaussianBlur(paper_grain, (3, 3), 0)

        # Apply subtle grain to all pixels
        for c in range(3):
            result[:, :, c] += paper_grain * 0.3  # Very subtle

        # 2. CRAYON TEXTURE - stronger in drawn areas
        # Detect non-white areas (actual drawing)
        gray = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)
        drawing_mask = gray < 250

        # Generate directional crayon strokes (not just random noise)
        crayon_texture = np.random.normal(0, 3.0, (h, w)).astype(np.float32)

        # Make it slightly directional (crayons have stroke direction)
        kernel_vertical = np.array([[1], [2], [1]]) / 4.0
        crayon_texture = cv2.filter2D(crayon_texture, -1, kernel_vertical)
        crayon_texture = cv2.GaussianBlur(crayon_texture, (3, 3), 0)

        # Apply stronger texture to drawn areas
        for c in range(3):
            channel = result[:, :, c]
            channel[drawing_mask] += crayon_texture[drawing_mask] * 0.15

        # 3. COLOR VARIATION - crayons aren't perfectly uniform
        # Add very subtle color shifts in drawn areas
        hsv = cv2.cvtColor(result.astype(np.uint8), cv2.COLOR_RGB2HSV).astype(np.float32)
        h_channel, s_channel, v_channel = cv2.split(hsv)

        # Add subtle saturation variation
        sat_noise = np.random.normal(0, 2, (h, w)).astype(np.float32)
        sat_noise = cv2.GaussianBlur(sat_noise, (5, 5), 0)
        s_channel[drawing_mask] += sat_noise[drawing_mask]
        s_channel = np.clip(s_channel, 0, 255)

        # Add subtle brightness variation
        val_noise = np.random.normal(0, 3, (h, w)).astype(np.float32)
        val_noise = cv2.GaussianBlur(val_noise, (5, 5), 0)
        v_channel[drawing_mask] += val_noise[drawing_mask]
